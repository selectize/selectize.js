...
0);b.preventDefault()});p(f,"mouseenter","*",function(){return a.onOptionHover.apply(a,arguments)});p(f,"mousedown","*",function(){return a.onOptionSelect.apply(a,arguments)});p(c,"mousedown","*:not(input)",function(){return a.onItemSelect.apply(a,arguments)});b=function(a){var b,c;if(!1!==d.data("grow")){a=a||window.event||{};b=d.val();if(a.type&&"keydown"===a.type.toLowerCase()&&(c=a.keyCode,97<=c&&122>=c||65<=c&&90>=c||48<=c&&57>=c||32==c))c=a.shiftKey,a=String.fromCharCode(a.keyCode),a=c?a.toUpperCase():
...
c||"textarea"===c)&&-1!==[16,8,46,27,37,39,9].indexOf(b.keyCode))return a.onKeyDown.apply(a,arguments)}},keyup:function(b){b.keyCode===v?a.isCtrlDown=!1:16===b.keyCode&&(a.isShiftDown=!1)},mousedown:function(b){if(a.isFocused)if(b.target===a.$dropdown[0]){var c=a.ignoreFocus;a.ignoreFocus=!0;window.setTimeout(function(){a.ignoreFocus=c;a.focus(!1)},0)}else!a.$control.has(b.target).length&&b.target!==a.$control[0]&&a.blur()}});g(window).on({resize:function(){a.isOpen&&a.positionDropdown.apply(a,arguments)}});
this.$input.attr("tabindex",-1).hide().after(this.$wrapper);g.isArray(this.settings.items)&&(this.setValue(this.settings.items),delete this.settings.items);this.updateOriginalInput();this.refreshItems();this.updatePlaceholder();this.isSetup=!0;if(this.settings.preload)this.onSearchChange("")};e.prototype.trigger=function(a){var b;"function"===typeof this.settings[a]&&(b=Array.prototype.slice.apply(arguments,[1]),this.settings[a].apply(this,b))};e.prototype.onKeyPress=function(a){if(this.isLocked)return a&&
