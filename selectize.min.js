...
a);b=1}}else if(1===a.nodeType&&a.childNodes&&!/(script|style)/i.test(a.tagName))for(j=0;j<a.childNodes.length;++j)j+=d(a.childNodes[j]);return b};return a.each(function(){d(this)})}},q=/Mac/.test(navigator.userAgent),v=q?18:17,m={a:"[a\u00c0\u00c1\u00c2\u00c3\u00c4\u00c5\u00e0\u00e1\u00e2\u00e3\u00e4\u00e5]",c:"[c\u00c7\u00e7]",e:"[e\u00c8\u00c9\u00ca\u00cb\u00e8\u00e9\u00ea\u00eb]",i:"[i\u00cc\u00cd\u00ce\u00cf\u00ec\u00ed\u00ee\u00ef]",n:"[n\u00d1\u00f1]",o:"[o\u00d2\u00d3\u00d4\u00d5\u00d5\u00d6\u00d8\u00f2\u00f3\u00f4\u00f5\u00f6\u00f8]",
...
this,b=arguments;window.clearTimeout(f);f=window.setTimeout(function(){c.apply(a,b)},d)};if(g.isArray(b.options))for(var h=b.valueField,j=0;j<b.options.length;j++)b.options[j].hasOwnProperty(h)&&(this.options[b.options[j][h]]=b.options[j]);else"object"===typeof b.options&&(g.extend(this.options,b.options),delete this.settings.options);this.settings.mode=this.settings.mode||(1===this.settings.maxItems?"single":"multi");"boolean"!==typeof this.settings.hideSelected&&(this.settings.hideSelected="multi"===
this.settings.mode);this.setup()};e.prototype.setup=function(){var a=this,b,c,d,f,h;f=this.$input.attr("tabindex")||"";this.$input.attr("tabindex",-1);b=g("<div>").addClass(this.settings.theme).addClass(this.settings.wrapperClass);c=g("<div>").addClass(this.settings.inputClass).addClass("items").toggleClass("has-options",!g.isEmptyObject(this.options)).appendTo(b);d=g('<input type="text">').appendTo(c).attr("tabindex",f);f=g("<div>").addClass(this.settings.dropdownClass).hide().appendTo(b);h=this.$input.css("display");
b.css({width:this.$input[0].style.width,display:h});h=this.settings.mode;b.toggleClass("single","single"===h);b.toggleClass("multi","multi"===h);(null===this.settings.maxItems||1<this.settings.maxItems)&&1===this.tagType&&this.$input.attr("multiple","multiple");this.settings.placeholder&&d.attr("placeholder",this.settings.placeholder);this.$wrapper=b;this.$control=c;this.$control_input=d;this.$dropdown=f;c.on("mousedown",function(b){b.currentTarget===a.$control[0]?d.trigger("focus"):a.focus(!0);b.preventDefault()});
p(f,"mouseenter","*",function(){return a.onOptionHover.apply(a,arguments)});p(f,"mousedown","*",function(){return a.onOptionSelect.apply(a,arguments)});p(c,"mousedown","*:not(input)",function(){return a.onItemSelect.apply(a,arguments)});b=function(a){var b,c;if(!1!==d.data("grow")){a=a||window.event||{};b=d.val();if(a.type&&"keydown"===a.type.toLowerCase()&&(c=a.keyCode,97<=c&&122>=c||65<=c&&90>=c||48<=c&&57>=c||32==c))c=a.shiftKey,a=String.fromCharCode(a.keyCode),a=c?a.toUpperCase():a.toLowerCase(),
...
b,c){var d,f,h;this.$activeOption&&this.$activeOption.removeClass("active");this.$activeOption=null;a=g(a);if(a.length&&(this.$activeOption=a.addClass("active"),b||!k(b)))a=this.$dropdown.height(),d=this.$activeOption.outerHeight(!0),b=this.$dropdown.scrollTop()||0,f=this.$activeOption.offset().top-this.$dropdown.offset().top+b,h=f-a+d,f+d>a-b?this.$dropdown.stop().animate({scrollTop:h},c?this.settings.scrollDuration:0):f<b&&this.$dropdown.stop().animate({scrollTop:f},c?this.settings.scrollDuration:
...
this.setActiveItem(null)};e.prototype.parseSearchTokens=function(a){a=g.trim(String(a||"").toLowerCase());if(!a||!a.length)return[];var b,c,d,f=[],h=a.split(/ +/);a=0;for(b=h.length;a<b;a++){c=(h[a]+"").replace(/([.?*+^$[\]\\(){}|-])/g,"\\$1");if(this.settings.diacritics)for(d in m)m.hasOwnProperty(d)&&(c=c.replace(RegExp(d,"g"),m[d]));f.push({string:h[a],regex:RegExp(c,"i")})}return f};e.prototype.getScoreFunction=function(a){var b=this,c=a.tokens,d=c.length?1===c.length?function(a){var b;a=String(a||
...
e.defaults={delimiter:",",persist:!0,diacritics:!0,create:!1,highlight:!0,openOnFocus:!0,maxOptions:1E3,maxItems:null,hideSelected:null,preload:!1,scrollDuration:60,loadThrottle:300,dataAttr:"data-data",sortField:null,sortDirection:"asc",valueField:"value",labelField:"text",searchField:["text"],mode:null,theme:"default",wrapperClass:"selectize-control",inputClass:"selectize-input",dropdownClass:"selectize-dropdown",load:null,score:null,onChange:null,onItemAdd:null,onItemRemove:null,onClear:null,onOptionAdd:null,
onOptionRemove:null,onDropdownOpen:null,onDropdownClose:null,onType:null,render:{item:null,option:null,option_create:null}};g.fn.selectize=function(a){var b=g.fn.selectize.defaults;a=a||{};return this.each(function(){var c,d,f,h,j,k,m,n,l=g(this);h=l[0].tagName.toLowerCase();if("string"===typeof a)c=l.data("selectize"),c[a].apply(c,Array.prototype.splice.apply(arguments,1));else{f=a.dataAttr||b.dataAttr;c={};c.placeholder=l.attr("placeholder");c.options={};c.items=[];if("select"===h){c.maxItems=l.attr("multiple")?
null:1;m=l.children();h=0;for(j=m.length;h<j;h++)n=g(m[h]),d=n.attr("value")||"",d.length&&(k=f&&n.attr(f)||{text:n.html(),value:d},"string"===typeof k&&(k=JSON.parse(k)),c.options[d]=k,n.is(":selected")&&c.items.push(d))}else if(d=g.trim(l.val()||""),d.length){f=d.split(a.delimiter||b.delimiter);h=0;for(j=f.length;h<j;h++)c.options[f[h]]={text:f[h],value:f[h]};c.items=f}c=new e(l,g.extend(!0,{},b,c,a));l.data("selectize",c);l.addClass("selectized")}})};g.fn.selectize.defaults=e.defaults;return e});
